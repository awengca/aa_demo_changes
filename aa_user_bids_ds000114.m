...
aap.directory_conventions.rawdatadir = '/Users/andrewweng/Data/aa_demo';

demodir = string(aap.directory_conventions.rawdatadir);
demodatadir = fullfile(aap.directory_conventions.rawdatadir,rawdir);


% check to see if {datafolder}/aa_demo exists - if not, create it
if exist(demodir, 'dir') == 7 && exist(demodatadir,'dir') == 7
    aas_log(aap,false,'INFO: test data directory found');
    aap.directory_conventions.rawdatadir = demodatadir;
    disp(string(demodir));
else
     aas_makedir(aap,demodir);
     disp('INFO: directory ' + demodir + ' created');
     disp('INFO: Downloading dataset to: '+ string(demodatadir));
...
disp(aap.directory_conventions.rawdatadir);
